...
/**
 * Annahme: torque und force wurden fÃ¼r d_t bereits berechnet
 */
...
/*******Momente aktualisieren********/
	//Anwenden wirkender KrÃ¤fte auf Linear und Angular Momentum
//	Vector3 Pdot = mForce;
//
//	mP += Pdot;
//	mCenterOfMass.setImpulse(mP);
...
//	mForce = Vector3(0,0,0);
	...
	Vector3 Ldot = mTorque;		//Momentänderung = Drehmoment
	mL += Ldot;					//Addiere Momentänderung

	mTorque = Vector3(0,0,0);

/*****Hilfsvariablen aktualisieren****/
	//Geschwindigkeit bereits geändert durch update

	//Update RotationMatrix
	mR = mQ.computeRotationMatrix();

	//Update aktuellen Trägheitstensor I_inv
	Matrix3 R_trans = mR;
	R_trans.transpose();
